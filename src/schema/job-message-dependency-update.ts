...
export const jobMessageDependencyUpdateInstall = (params: {
    schema: SqlRefNode
}) => {
    return [
        sql `
            CREATE FUNCTION ${params.schema}.job_message_dependency_update(
                p_id UUID
            ) RETURNS VOID AS $$
            DECLARE
                v_now TIMESTAMP;
            ...
            BEGIN
                ...
                INSERT INTO ${params.schema}.job (
                    ...
                    ${valueNode(JobType.MESSAGE_DEPENDENCY_UPDATE)},
                    v_params,
                    ${valueNode(false)},
                    v_now
                );
